...
NSString * walmartstorelocatorweek4$ModuleRequireFormat = @"(function(exports){"
...
@implementation walmartstorelocatorweek4Object
...
-(void)dealloc
{
	...
	RELEASE_TO_NIL(_walmartstorelocatorweek4);
	OSSpinLockLock(&krollBridgeRegistryLock);
...
{
	// create walmartstorelocatorweek4 global object
    NSAutoreleasePool* pool = [[NSAutoreleasePool alloc] init];
    
    // Load the "walmartstorelocatorweek4" object into the global scope
	NSString *basePath = (url==nil) ? [TiHost resourcePath] : [[[url path] stringByDeletingLastPathComponent] stringByAppendingPathComponent:@"."];
	_walmartstorelocatorweek4 = [[walmartstorelocatorweek4Object alloc] initWithContext:kroll host:host context:self baseURL:[NSURL fileURLWithPath:basePath]];
	
	TiContextRef jsContext = [kroll context];
	TiValueRef tiRef = [KrollObject toValue:kroll value:_walmartstorelocatorweek4];
	
	NSString *_walmartstorelocatorweek4NS = [NSString stringWithFormat:@"T%sanium","it"];
	TiStringRef prop = TiStringCreateWithCFString((CFStringRef) _walmartstorelocatorweek4NS);
	...
	if (preload!=nil)
	{
		...
		{
			KrollObject *ti = (KrollObject*)[_walmartstorelocatorweek4 valueForKey:name];
...
-(id)loadCommonJSModule:(NSString*)code withSourceURL:(NSURL *)sourceURL
{
	...
	NSString *js = [[NSString alloc] initWithFormat:walmartstorelocatorweek4$ModuleRequireFormat, dirname, filename,code];
...
loadNativeJS:
	...
	if (data!=nil)
	{
		...
		if (![wrapper respondsToSelector:@selector(replaceValue:forKey:notification:)]) {
			...
			@throw [NSException exceptionWithName:@"org.walmartstorelocatorweek4.kroll" 
