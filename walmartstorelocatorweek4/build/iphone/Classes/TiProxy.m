...
//Rare exceptions to indicate a bug in the _walmartstorelocatorweek4 code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_walmartstorelocatorweek4 = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_walmartstorelocatorweek4]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.walmartstorelocatorweek4.module"
