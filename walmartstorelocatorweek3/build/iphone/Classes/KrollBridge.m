...
NSString * walmartstorelocatorweek3$ModuleRequireFormat = @"(function(exports){"
...
@implementation walmartstorelocatorweek3Object
...
-(void)dealloc
{
	...
	RELEASE_TO_NIL(_walmartstorelocatorweek3);
	OSSpinLockLock(&krollBridgeRegistryLock);
...
{
	// create walmartstorelocatorweek3 global object
    NSAutoreleasePool* pool = [[NSAutoreleasePool alloc] init];
    
    // Load the "walmartstorelocatorweek3" object into the global scope
	NSString *basePath = (url==nil) ? [TiHost resourcePath] : [[[url path] stringByDeletingLastPathComponent] stringByAppendingPathComponent:@"."];
	_walmartstorelocatorweek3 = [[walmartstorelocatorweek3Object alloc] initWithContext:kroll host:host context:self baseURL:[NSURL fileURLWithPath:basePath]];
	
	TiContextRef jsContext = [kroll context];
	TiValueRef tiRef = [KrollObject toValue:kroll value:_walmartstorelocatorweek3];
	
	NSString *_walmartstorelocatorweek3NS = [NSString stringWithFormat:@"T%sanium","it"];
	TiStringRef prop = TiStringCreateWithCFString((CFStringRef) _walmartstorelocatorweek3NS);
	...
	if (preload!=nil)
	{
		...
		{
			KrollObject *ti = (KrollObject*)[_walmartstorelocatorweek3 valueForKey:name];
...
-(id)loadCommonJSModule:(NSString*)code withSourceURL:(NSURL *)sourceURL
{
	...
	NSString *js = [[NSString alloc] initWithFormat:walmartstorelocatorweek3$ModuleRequireFormat, dirname, filename,code];
...
loadNativeJS:
	...
	if (data!=nil)
	{
		...
		if (![wrapper respondsToSelector:@selector(replaceValue:forKey:notification:)]) {
			...
			@throw [NSException exceptionWithName:@"org.walmartstorelocatorweek3.kroll" 
