...
//Rare exceptions to indicate a bug in the _walmartstorelocatorweek3 code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_walmartstorelocatorweek3 = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_walmartstorelocatorweek3]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.walmartstorelocatorweek3.module"
